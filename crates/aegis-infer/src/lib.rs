...
//!   owned by `aegis-vision`/`-audio`/`-text`. The actual ONNX run goes through
...
//! ```no_run
//! use std::sync::Arc;
//! use aegis_infer::{DefaultOffloadRouter, OffloadRouter, NullAnalyzer};
//! use aegis_infer::client::{OffloadClient, ClientTlsIdentity};
//! use aegis_core::detect_device_profile;
//!
//! # async fn run(tls: ClientTlsIdentity) -> aegis_core::Result<()> {
