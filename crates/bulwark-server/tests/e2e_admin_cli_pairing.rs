//! End-to-end provisioning test through the `bulwark_admin` executable.
//!
...
async fn admin_cli_creates_pair_code_and_child_redeems_it() {
    ...
    let admin = admin_bin();
    let base_env = [("BULWARK_ADMIN_ENDPOINT", server.endpoint.as_str())];

    let created = run_admin(
        &admin,
        &["create-account", "guardian-cli@example.com", "Guardian CLI"],
        &[
            ("BULWARK_ADMIN_ENDPOINT", server.endpoint.as_str()),
            ("BULWARK_ADMIN_PASSWORD", "password123"),
        ],
    );
...
fn admin_bin() -> PathBuf {
    if let Some(path) = option_env!("CARGO_BIN_EXE_bulwark_admin") {
        return PathBuf::from(path);
    }
    let exe = std::env::current_exe().expect("current test exe");
    let deps = exe.parent().expect("deps dir");
    let debug = deps.parent().expect("debug dir");
    debug.join(format!("bulwark_admin{}", std::env::consts::EXE_SUFFIX))
}

fn run_admin(bin: &PathBuf, args: &[&str], envs: &[(&str, &str)]) -> String {
    let mut cmd = Command::new(bin);
    cmd.args(args);
    cmd.env_remove("BULWARK_ADMIN_PASSWORD")
        .env_remove("BULWARK_GUARDIAN_TOKEN")
        .env_remove("BULWARK_CLUSTER_CA")
        .env_remove("BULWARK_ADMIN_ENDPOINT");
    for (k, v) in envs {
        cmd.env(k, v);
    }
    let output = cmd.output().expect("spawn bulwark_admin");
    if !output.status.success() {
        panic!(
            "bulwark_admin {:?} failed with {}\nstdout:\n{}\nstderr:\n{}",
            args,
            output.status,
