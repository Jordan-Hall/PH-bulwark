//
//  AegisEngine.swift — a safe Swift wrapper over the aegis-apple-ffi C ABI.
//
...
public final class AegisEngine {
    ...
    public init?() {
        guard let h = aegis_apple_engine_new() else { return nil }
        self.handle = h
    }

    deinit {
        aegis_apple_engine_free(handle)
    }
    ...
    public func classify(_ text: String, threadID: String = "") -> (action: AegisAction, category: Int32) {
            ...
            threadBytes.withUnsafeBufferPointer { threadBuf in
                textBuf.baseAddress!.withMemoryRebound(to: CChar.self, capacity: textBuf.count) { textPtr in
                    threadBuf.baseAddress!.withMemoryRebound(to: CChar.self, capacity: max(threadBuf.count, 1)) { threadPtr in
                        aegis_apple_classify_text(handle,
                                                  textPtr,
                                                  textBuf.count,
        ...
        return (AegisAction(rawValue: code) ?? .allow, category)
    }
}
