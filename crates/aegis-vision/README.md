...
```sh
cargo build -p aegis-vision
cargo test  -p aegis-vision
```
...
$env:AEGIS_NSFW_MODEL = "C:\models\nsfw.onnx"      # PowerShell
export AEGIS_NSFW_MODEL=/opt/aegis/models/nsfw.onnx # bash

cargo build -p aegis-vision --features onnx
cargo test  -p aegis-vision --features onnx
```
...
`AEGIS_NSFW_MODEL` at. Known-good options:
...
```rust
use aegis_vision::{VisionAnalyzer, VisionConfig};

// Picks OnnxScorer when `--features onnx` is on AND a loadable model is
// configured (via cfg.model_path or AEGIS_NSFW_MODEL); otherwise the safe stub.
...
* `src/onnx.rs` — `ort`-backed `OnnxScorer` (CPU EP, deterministic). Compiled
  ...
  runs only when `AEGIS_NSFW_MODEL` points at an existing file.
