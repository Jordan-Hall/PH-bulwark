...
//  BulwarkFilter — the Apple child shell's Network Extension content filter.
//
...
//  text where available, asks the shared Rust core (via the bulwark-apple-ffi C
//  ABI) for a verdict, and returns .allow() or .drop() accordingly — and posts a
//  REDACTED local notification on a block.
//
//  SCOPE — Apple platform + Bulwark policy. This provider can ONLY see the flows
...
//  Bulwark by design. This is FILTER + ALERTS, transparently.
//
...
/// Action codes returned by `bulwark_apple_classify_text` (see bulwark_apple.h).
private enum BulwarkAction: Int32 {
    case allow = 0   // BULWARK_APPLE_ALLOW
    case warn  = 1   // BULWARK_APPLE_WARN
    case block = 2   // BULWARK_APPLE_BLOCK
}

final class FilterDataProvider: NEFilterDataProvider {

    /// Non-sensitive diagnostics only (counts/categories — never content).
    private let log = Logger(subsystem: "co.uk.predatorhunters.bulwark.filter", category: "provider")
    ...
    override func startFilter(completionHandler: @escaping (Error?) -> Void) {
        ...
        engine = bulwark_apple_engine_new()
        if engine == nil {
            log.error("bulwark engine failed to initialize; filter will fail open (allow)")
        }
    ...
    override func handleNewFlow(_ flow: NEFilterFlow) -> NEFilterNewFlowVerdict {
        ...
        // here (and Bulwark does NOT attempt covert interception on Apple); such
    ...
    private func verdict(for bytes: Data, flow: NEFilterFlow) -> NEFilterDataVerdict {
        ...
        switch BulwarkAction(rawValue: action) ?? .allow {
        case .allow:
            return .allow()
        case .warn:
            ...
            threadBytes.withUnsafeBufferPointer { threadBuf in
                textBuf.baseAddress!.withMemoryRebound(to: CChar.self, capacity: textBuf.count) { textPtr in
                    threadBuf.baseAddress!.withMemoryRebound(to: CChar.self, capacity: threadBuf.count) { threadPtr in
                        bulwark_apple_classify_text(engine,
                                                  textPtr,
                                                  textBuf.count,
    ...
    private func postBlockNotification(category: Int32, dropped: Bool) {
        ...
        content.title = dropped ? "Bulwark blocked content" : "Bulwark flagged content"
    ...
    /// Content-free category label (mirrors BulwarkAppleCategory in bulwark_apple.h).
