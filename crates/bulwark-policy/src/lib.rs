//! # bulwark-policy — `Verdict → Action`, deterministically.
//!
...
//! severity + reason) via [`Policy::evaluate`], because callers (bulwark-client,
//! bulwark-store, bulwark-alert) want the alert/severity/reason in one pass rather
...
use bulwark_proto::v1::{Action, AlertKind, Category, Severity, SourceChannel, Verdict};
...
pub trait PolicyEngine: Send + Sync {
    ...
    /// `None` = no alert (e.g. plain LOG). bulwark-alert dedupes downstream.
    fn alert_for(
        &self,
        verdict: &Verdict,
        action: Action,
...
impl Policy {
    ...
    /// Load config (defaults → optional TOML file → `BULWARK_POLICY_*` env) and
...
mod tests {
    use super::*;
    use bulwark_proto::v1::{Action, AlertKind, Category, Severity, SourceChannel};
    ...
    fn load_defaults_without_file_or_env() {
        // No path, no BULWARK_POLICY_* env in test → pure defaults, validated.
