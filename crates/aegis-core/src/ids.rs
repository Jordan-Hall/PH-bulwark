...
//!
//! These wrap the stringly-typed ids that flow through the `aegis.v1` wire
...
//! in `aegis-proto`; we re-export `DeviceId` here so downstream crates have a
//! single `aegis_core` import surface for all four ids. [`ThreadId`] and
...
// Re-export the proto-owned device identifier so `aegis_core::DeviceId` and
// `aegis_proto::DeviceId` are the same type (no conversion needed at boundaries).
pub use aegis_proto::DeviceId;
...
mod tests {
    ...
    fn device_id_is_reexported_proto_type() {
        // Same underlying type as aegis_proto::DeviceId — proves the re-export.
        let d: DeviceId = aegis_proto::DeviceId("dev-1".to_owned());
