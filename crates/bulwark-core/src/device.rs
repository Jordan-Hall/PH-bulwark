...
//!
//! Produces a [`DeviceProfile`] (the `bulwark.v1` wire message) that
//! `bulwark-infer`'s [`OffloadRouter`] uses to negotiate local-vs-cluster routing
...
use bulwark_proto::v1::{DeviceProfile, ExecutionProvider};
use sysinfo::System;
...
///
/// An explicit `BULWARK_GPU` override always wins (operators/tests). Otherwise it
...
fn detect_gpu() -> String {
    let env = std::env::var("BULWARK_GPU")
        .unwrap_or_default()
        .trim()
        .to_owned();
    if !env.is_empty() {
