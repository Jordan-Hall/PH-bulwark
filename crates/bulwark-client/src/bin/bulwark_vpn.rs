//! `bulwark_vpn` — a runnable, **transparent VPN** entrypoint (desktop: Windows /
//! Linux / macOS).
//!
...
fn main() {
    eprintln!("bulwark_vpn: VPN mode is desktop-only (Windows/Linux/macOS). Mobile uses the native VpnService / NetworkExtension.");
}
...
async fn main() -> anyhow::Result<()> {
    ...
    if !bulwark_net::is_elevated() {
        eprintln!("bulwark_vpn needs elevation (TUN adapter + default route).");
        eprintln!(
            "Re-run as administrator/root:\n  {}",
            bulwark_net::elevation_command()
        );
        std::process::exit(1);
    }
    if !bulwark_net::wintun_available() {
    ...
    // (see bulwark_proxy for the rationale).
    let cfg = ClientConfig {
        device_id: "bulwark-vpn-local".to_string(),
    ...
    let shutdown = bulwark_net::CancellationToken::new();
    let vpn_token = shutdown.clone();
    let mut vpn = tokio::spawn(async move {
        bulwark_net::run_vpn(bulwark_net::VpnConfig::default(), vpn_token).await
    });

    println!("bulwark_vpn: bringing up the transparent VPN…");
        ...
        res = &mut vpn => {
            ...
            return match res {
                ...
                Ok(Err(e)) => {
                    ...
                    eprintln!("Use proxy mode instead: run `bulwark_proxy` (no admin), trust the CA, done.");
                    std::process::exit(1);
                }
                Err(join) => {
