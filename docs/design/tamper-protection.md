...
How Bulwark keeps the **child** app from being removed or disabled without an
...
> where the child can see Bulwark is active. The same techniques applied covertly to
> an adult's device would be stalkerware; Bulwark never does that (visible
...
- The child app sends a periodic **`Tamper.Heartbeat`** (`crates/bulwark-proto`) with
...
- The cluster (`bulwark-server::tamper::TamperService`) turns a self-reported
...
- The desktop binaries (`bulwark_proxy`, `bulwark_vpn`) run the reporter
  (`bulwark-client::tamper::run_heartbeats`); Android reports via
...
- **`BulwarkDeviceAdminReceiver`** (`co.predatorhunters.bulwark.admin`): while it is an
...
- **Uninstall-guard** (`BulwarkAccessibilityService`): watches the package
  installer / Settings (`TYPE_WINDOW_STATE_CHANGED`); if the visible screen is an
  attempt to uninstall Bulwark, it raises `APP_UNINSTALL_ATTEMPT` and navigates Home.
...
`adb shell dpm set-device-owner co.predatorhunters.bulwark/.admin.BulwarkDeviceAdminReceiver`.
`BulwarkDeviceAdminReceiver` handles `onProfileProvisioningComplete` (QR/NFC/zero-touch)
...
- Auto-start scaffolding ships under `deploy/`:
  - **Windows** — `deploy/windows/install-bulwark-service.ps1` (admin) installs the
    `bulwark_svc` **SCM service** (LocalSystem, auto-start) and **locks its DACL**
    ...
    `bulwark_proxy.exe` and respawns it if killed. *Refinement (documented):* a
    ...
    `CreateProcessAsUserW` (isolated Win32 in `bulwark-net`), or host the in-process
    transparent VPN once its data path lands. `install-bulwark-autostart.ps1` remains
    the no-service logon-task fallback.
  - **macOS** — `deploy/macos/co.predatorhunters.bulwark.proxy.plist` (a LaunchDaemon for
  ...
  - **Linux** — `deploy/linux/bulwark-proxy.service` (systemd).
...
routes (no Bulwark app code can change this):
...
- **Factory reset / recovery / re-flash defeats anything** short of devices
  **enrolled as managed from setup** (Android zero-touch / Knox, Apple ABM/DEP),
  ...
  org/family. Bulwark now supports Device Owner *provisioning* (QR / NFC / dev `dpm`;
