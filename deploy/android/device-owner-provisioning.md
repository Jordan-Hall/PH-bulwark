...
Provisioning the Bulwark child app as **Device Owner** is what makes anti-removal
...
Component: `co.predatorhunters.bulwark/.admin.BulwarkDeviceAdminReceiver`
...
account, no secondary users/profiles), Bulwark installed.
...
- Tear-down (debuggable builds): guardian un-enroll should call `Lockdown.release`
  first; then `adb shell dpm remove-active-admin co.predatorhunters.bulwark/.admin.BulwarkDeviceAdminReceiver`.
...
joins Wi-Fi, downloads the APK, verifies the signing-cert checksum, installs Bulwark,
...
```json
{
  "android.app.extra.PROVISIONING_DEVICE_ADMIN_COMPONENT_NAME": "co.predatorhunters.bulwark/.admin.BulwarkDeviceAdminReceiver",
  "android.app.extra.PROVISIONING_DEVICE_ADMIN_PACKAGE_DOWNLOAD_LOCATION": "https://dl.predatorhunters.co.uk/bulwark/bulwark-child-release.apk",
  ...
  "android.app.extra.PROVISIONING_ADMIN_EXTRAS_BUNDLE": {
    "co.predatorhunters.bulwark.family_id": "fam_xxx",
    "co.predatorhunters.bulwark.child_id": "child_xxx",
    "co.predatorhunters.bulwark.cluster_endpoint": "https://cluster.predatorhunters.co.uk:8443"
  }
}
```

Key points:
- **Component name** must be exactly `co.predatorhunters.bulwark/.admin.BulwarkDeviceAdminReceiver`.
- **Signature checksum** is the **URL-safe, no-padding Base64 of the signing
  ...
  APK:
  ```sh
  apksigner verify --print-certs bulwark-child-release.apk   # take the cert SHA-256
...
```sh
qrencode -o bulwark-do-qr.png -d 300 -s 6 < provisioning.min.json
```
...
- **Zero-touch**: register the device serial/IMEI in the zero-touch portal against an
  Bulwark configuration pointing at the same component + download location;
