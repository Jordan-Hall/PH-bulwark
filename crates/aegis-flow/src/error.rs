//! Crate-local error type, convertible into the shared [`aegis_core::Error`].
//!
//! The [`FlowClassifier`](crate::FlowClassifier) trait (docs/design/interfaces.md)
//! returns [`aegis_core::Result`], so [`FlowError`] exists only to give the
//! classification/buffering internals precise, matchable variants; it `From`-converts
//! into [`aegis_core::Error`] at the trait boundary.
