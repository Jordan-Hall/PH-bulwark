...
//! Bulwark ships **no telemetry** (PLAN §3) — nothing here phones home, exports
...
pub struct LoggingConfig {
    ...
    /// Default directive used when `RUST_LOG` / `BULWARK_LOG` is unset
    /// (e.g. `"info,bulwark_net=debug"`).
...
///
/// The env filter reads `BULWARK_LOG` first, then falls back to `RUST_LOG`, then
