...
//! variables (`BULWARK_POLICY_*`) without a parallel DTO layer.
...
impl PolicyConfig {
    ...
    ///   3. environment variables prefixed `BULWARK_POLICY_`.
    ///
    ...
    pub fn load(path: Option<&str>) -> Result<Self> {
        ...
        fig = fig.merge(Env::prefixed("BULWARK_POLICY_").split("__"));
