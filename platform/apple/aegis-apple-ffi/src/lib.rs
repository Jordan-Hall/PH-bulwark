//! # aegis-apple-ffi — a stable C ABI over the shared Aegis content-safety core
//!
...
//!
//! ## Scope guardrail (Apple platform + Aegis policy)
...
// on (`aegis-core`, `aegis-text`, `aegis-policy`, `aegis-proto`) each
...
use aegis_policy::{AgeProfile, Policy, PolicyContext};
use aegis_proto::v1::{Action, Category, SourceChannel, TextSpan, Verdict};
use aegis_proto::DeviceId;
use aegis_text::TextAnalyzer;

/// Stable action codes returned across the C ABI by
/// [`ffi::aegis_apple_classify_text`]. Deliberately tiny and language-neutral.
///
/// These collapse the richer [`aegis_proto::v1::Action`] ladder (ALLOW / LOG /
...
/// [`ffi::aegis_apple_classify_text`]. Mirrors [`aegis_proto::v1::Category`] so
...
/// The boxed engine handed across the C ABI as an opaque `*mut AegisEngine`.
///
...
mod tests {
    ...
    #[test]
    fn engine_builds() {
        assert!(AegisEngine::new().is_some());
    }

    #[test]
    fn benign_text_is_allowed_and_safe() {
        let e = AegisEngine::new().unwrap();
