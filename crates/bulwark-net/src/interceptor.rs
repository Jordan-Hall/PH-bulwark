...
//! args, ownership), returning the shared [`bulwark_core::Result`]. The supporting
//! types `CapturedFlow` / `InterceptDecision` / `FlowPayload` are defined here
//! because they are not in `bulwark-proto` (proto has the `SourceChannel` enum,
...
// The captured-flow vocabulary is now CANONICAL in `bulwark-core::flow` so this
// crate and `bulwark-flow` share ONE definition (no drift). Re-exported here so the
// crate's public API (`bulwark_net::CapturedFlow`, …) is unchanged for callers.
pub use bulwark_core::flow::{CapturedFlow, FlowPayload, HttpHead, InterceptDecision};
...
/// Concrete `bulwark-net` interceptor. Construct with [`NetInterceptor::new`]
...
impl NetInterceptor {
    ...
    fn convert(flow: ProxyFlow) -> CapturedFlow {
        ...
        // the `content-type` header and bulwark-flow's content-type fast-path
        // engages (instead of relying solely on magic-byte sniffing).
        let mut headers = Vec::new();
        if let Some(ct) = &flow.content_type {
            headers.push(bulwark_core::flow::Header {
...
impl Interceptor for NetInterceptor {
    async fn start(&self) -> CoreResult<()> {
        // 1. Open + bring the TUN device up (lazily — opening may need the driver).
        {
            ...
            let mut tun = open_tun().map_err(bulwark_core::Error::from)?;
            let tun_cfg = TunConfig::default();
            tun.up(&tun_cfg).map_err(bulwark_core::Error::from)?;
        ...
        let listen: std::net::SocketAddr = self
            ...
            .proxy_listen
            .parse()
            .map_err(|e| bulwark_core::Error::Config(format!("bad proxy_listen: {e}")))?;

        let tx =
            self.flow_tx.lock().await.take().ok_or_else(|| {
    ...
    async fn shutdown(&self) -> CoreResult<()> {
        ...
        self.quic.remove_rule().map_err(bulwark_core::Error::from)?;
        // 3. Tear down the TUN device — MUST restore host routing (no blackhole).
        {
            ...
            if let Some(dev) = tun.as_mut() {
                dev.close().map_err(bulwark_core::Error::from)?;
            }
            *tun = None;
        }
