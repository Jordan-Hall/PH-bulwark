//! Crate-local error type for `bulwark-net`, convertible into the shared
//! [`bulwark_core::Error`].
//!
//! The [`Interceptor`](crate::Interceptor) trait (interfaces.md) returns
//! [`bulwark_core::Result`], so [`NetError`] implements `Into<bulwark_core::Error>`
...
/// Result alias used throughout `bulwark-net`.
...
/// layer's specific variants are collapsed onto the coarse `bulwark_core::Error`
/// surface the trait boundary speaks; the detail survives in the message.
impl From<NetError> for bulwark_core::Error {
    fn from(e: NetError) -> Self {
        match e {
            NetError::Io(io) => bulwark_core::Error::Io(io),
            ...
            NetError::SharedCaRejected(_) | NetError::KeyStore(_) => {
                bulwark_core::Error::Config(e.to_string())
            }
            other => bulwark_core::Error::Other(anyhow::Error::new(other)),
        }
    }
}
...
mod tests {
    ...
    fn shared_ca_rejection_maps_to_config_error() {
        let e: bulwark_core::Error = NetError::SharedCaRejected("bundled key".into()).into();
        assert!(matches!(e, bulwark_core::Error::Config(_)));
    }

    #[test]
    fn keystore_failure_is_loud() {
        // Fail-closed: a keystore failure must not vanish into a soft variant.
        let e: bulwark_core::Error = NetError::keystore("DPAPI unavailable").into();
        assert!(matches!(e, bulwark_core::Error::Config(_)));
    }
    ...
    fn io_roundtrips() {
        ...
        let e: bulwark_core::Error = NetError::from(io).into();
        assert!(matches!(e, bulwark_core::Error::Io(_)));
    }
}
