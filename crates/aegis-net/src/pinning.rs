...
//! that this host/app must be routed to the **on-device agent** (aegis-agent
...
/// this to `aegis-agent` so the host/app is covered by OCR instead, and to the
...
impl PinningRegistry {
    ...
    /// caller forwards to aegis-agent + the coverage dashboard. Honours the
