...
//! `aegis-vision` / `aegis-audio` / `aegis-video` / `aegis-text` /
//! `aegis-supervision` (server-side, heavy models) and `aegis-infer`
//! (client-side, tiny first-pass). `aegis-server` dispatches by
...
use crate::Result;
use aegis_proto::v1::{AnalysisBatch, AnalysisRequest, MediaKind, Verdict, VerdictBatch};
