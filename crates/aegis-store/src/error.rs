...
//! [`aegis_core::Error`] so the [`crate::Store`] trait can return
//! `aegis_core::Result<T>` per `docs/design/interfaces.md`.
//!
//! Backend driver errors (`rusqlite`, `sqlx`) are wrapped here and then funnel
//! into [`aegis_core::Error::Other`] via `anyhow` at the trait boundary, which
...
impl From<StoreError> for aegis_core::Error {
    fn from(e: StoreError) -> Self {
        aegis_core::Error::Other(anyhow::Error::new(e))
    }
}
...
mod tests {
    ...
    fn converts_into_core_error() {
        let e: aegis_core::Error = StoreError::backend("disk full").into();
