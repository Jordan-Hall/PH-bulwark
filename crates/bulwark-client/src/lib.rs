//! bulwark-client — the device-side orchestration loop.
//!
...
//! ```
//!
//! NOTE: `CapturedFlow`/`FlowPayload` are now canonical in `bulwark-core::flow`
//! (re-exported by `bulwark-net` and `bulwark-flow`), so the interceptor's output
//! feeds the classifier directly — no adapter needed. The per-crate `Analyzer`
//! trait copies still want hoisting into `bulwark-core` (docs/integration-todo.md §1).
//!
...
use bulwark_policy::PolicyEngine; // trait providing Policy::decide / Policy::alert_for
...
/// `bulwark-vision` default threshold so the device-side fast path and any cluster
...
/// `client_ca_pem`.
///
/// Contract: `BULWARK_CLIENT_CERT` / `BULWARK_CLIENT_KEY` / `BULWARK_CLIENT_CA` (PEM
/// file paths) + `BULWARK_CLUSTER_DOMAIN` (expected server name / SNI).
...
pub struct Pipeline {
    ...
    /// AND a model at `BULWARK_NSFW_MODEL`; otherwise the fail-OPEN stub (score 0.0
    ...
    /// [`Pipeline::with_offload`]; the concrete `bulwark-infer` `DefaultOffloadRouter`
...
impl Pipeline {
    ...
    /// Route `VideoSegment` units through `analyzer` (any [`bulwark_core::Analyzer`]
    ...
    ///
    /// Builds an [`bulwark_video::VideoAnalyzer`] whose demuxer depends on features:
    ...
    /// `router` — the `bulwark-infer` [`OffloadRouter`] door to the cluster's
    /// `Analysis` service. The composition root builds the concrete
    /// [`bulwark_infer::DefaultOffloadRouter`] (which owns the mTLS `OffloadClient`)
    ...
    /// heavy media would route to the cluster via bulwark-infer's OffloadRouter
    ...
    async fn analyze(&self, unit: &AnalysisUnit) -> Verdict {
        match unit {
            ...
            // AUDIO → offload to the cluster's Analysis service via bulwark-infer's
        ...
        match router.analyze(req).await {
            ...
            Err(e) => {
                tracing::warn!(error = %e, kind = ?kind,
                    "bulwark-client: offload analysis failed; failing open");
    ...
    async fn analyze_video(
        ...
        match analyzer.analyze(req).await {
            Ok(v) => v,
            Err(e) => {
                tracing::warn!(error = %e, "bulwark-client: video analysis failed; failing open");
    ...
    fn analyze_image(&self, media: &InlineMedia) -> Verdict {
        ...
        // SEAM: a persisted, parent-APPROVED allowlist (bulwark-policy::allowlist,
    ...
    pub async fn run(&self, interceptor: Arc<dyn Interceptor>) -> Result<()> {
        // net + flow now share bulwark_core::flow::CapturedFlow, so the
...
fn span_ts(span: &bulwark_proto::v1::TextSpan) -> i64 {
    let _ = span;
    span_now()
}
...
/// * `--features onnx` + a model at `BULWARK_NSFW_MODEL` that loads → real
///   [`OnnxScorer`](bulwark_vision::onnx::OnnxScorer).
/// * otherwise → the fail-OPEN [`StubScorer`](bulwark_vision::StubScorer) (0.0 →
...
fn build_nsfw_scorer() -> Box<dyn Scorer> {
        ...
        match bulwark_vision::onnx::OnnxScorer::from_env(NSFW_INPUT_SIZE) {
            Ok(scorer) => {
                tracing::info!(
                    ...
                    "bulwark-client: local ONNX NSFW scorer active"
                );
                return Box::new(scorer);
            }
            Err(e) => {
                tracing::warn!(
                    "bulwark-client: no usable NSFW model ({e}); image scoring fails OPEN (stub)"
                );
            }
        }
    }
    ...
    {
        tracing::warn!(
            "bulwark-client: built without `onnx`; image scoring fails OPEN (stub). \
             Rebuild with --features onnx and set {} for real blocking.",
            bulwark_vision::MODEL_PATH_ENV
        );
    }
...
mod tests {
    ...
    /// analyzer; the real store/CSAM-gating is covered in `bulwark-video`.
    #[tokio::test]
    ...
    async fn video_segment_buffer_ticket_is_released_after_analysis() {
        use bulwark_proto::v1::SourceChannel;
        ...
        impl Interceptor for NoopInterceptor {
            ...
            async fn next_flow(&self) -> Result<Option<bulwark_flow::CapturedFlow>> {
                Ok(None)
            }
        ...
        let mk_flow = || bulwark_flow::CapturedFlow {
            flow_id: 7,
            source_channel: SourceChannel::Web,
            app_or_host: "cdn.example.com".into(),
            readable: true,
            payload: bulwark_flow::FlowPayload::Http(bulwark_flow::HttpHead {
                method: Some("GET".into()),
                path: Some("/clip.mp4".into()),
                status: Some(200),
                headers: vec![bulwark_flow::Header {
    ...
    impl OffloadRouter for StubOffload {
        async fn negotiate(
            &self,
            _p: bulwark_proto::v1::DeviceProfile,
        ) -> bulwark_infer::Result<bulwark_proto::v1::OffloadPolicy> {
            unreachable!("not used in analyze() tests")
        }
        fn route(&self, _k: MediaKind, _rtt: u32, _q: u32) -> bulwark_infer::Route {
            bulwark_infer::Route::Cluster
        }
        async fn analyze(&self, req: AnalysisRequest) -> bulwark_infer::Result<Verdict> {
        ...
        async fn refresh(
            &self,
            _r: bulwark_proto::v1::RefreshOffloadRequest,
        ) -> bulwark_infer::Result<bulwark_proto::v1::OffloadPolicy> {
            unreachable!("not used in analyze() tests")
        }
    }
    ...
    async fn audio_offload_error_fails_open() {
        ...
        impl OffloadRouter for ErrOffload {
            ...
            fn route(&self, _k: MediaKind, _r: u32, _q: u32) -> bulwark_infer::Route {
                bulwark_infer::Route::Cluster
            }
            async fn analyze(&self, _req: AnalysisRequest) -> bulwark_infer::Result<Verdict> {
                Err(bulwark_core::Error::Ipc("cluster down".into()))
            }
            async fn refresh(
                &self,
                _r: bulwark_proto::v1::RefreshOffloadRequest,
    ...
    fn load_cluster_tls_from_env_all_or_nothing() {
        ...
        std::env::set_var("BULWARK_CLIENT_CERT", &cert);
        std::env::set_var("BULWARK_CLIENT_KEY", &key);
        std::env::set_var("BULWARK_CLIENT_CA", &ca);
        std::env::set_var("BULWARK_CLUSTER_DOMAIN", "cluster.example");
