//! Shared in-process flow vocabulary — the canonical home for the Rust hand-off
//! types named in `docs/design/interfaces.md` (`CapturedFlow`, `FlowPayload`,
//! `InterceptDecision`, `AnalysisUnit`).
//!
//! These are **not** part of the `bulwark.v1` wire contract — they carry transient
//! bytes/handles that never cross a process or network boundary. They live here
//! (not in `bulwark-proto`, not duplicated per crate) so `bulwark-net` (which
//! *produces* a [`CapturedFlow`]) and `bulwark-flow` (which turns it into
//! [`AnalysisUnit`]s) link one definition. Each [`AnalysisUnit`] maps 1:1 onto an
//! `AnalysisRequest` once the router fills `device_id`/`ts`/`request_id`.
//!
//! `bytes::Bytes` is used for payload/segment handles so the ring buffer can hold
//! and clone segments cheaply (ref-counted, no copy).

use bytes::Bytes;

use bulwark_proto::v1::{InlineMedia, SourceChannel, TextSpan};

/// One header line from the request or response head of a captured flow.
///
/// Names are kept lowercased for case-insensitive lookup (HTTP header names are
...
pub struct CapturedFlow {
    ...
    /// **refine** it (a `WEB` flow whose body is an HLS manifest becomes
...
pub enum AnalysisUnit {
    ...
    Audio(InlineMedia),
    /// A buffered video segment → `MediaKind::VIDEO`.
    VideoSegment {
        ...
        /// `Action` can be applied back to the held bytes. `None` = pass-through.
        segment_id: Option<u64>,
    },
}

impl AnalysisUnit {
    /// The media kind this unit represents, for logging / analyzer dispatch.
