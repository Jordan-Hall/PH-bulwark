...
const NEVER: [(&str, &str, &str); 4] = [
    ("eye-off", "The screen", "We never record or stream what a child sees or types. The screen is read on the device, in the moment, for one job, and then it is gone."),
    ...
    ("lock", "Anyone else", "The tools run on one child's own devices, with consent. Never on the people around them, never on adults, and never in secret."),
];

/// (term, definition) — the principles.
const PRINCIPLES: [(&str, &str); 6] = [
    ("On device by default", "The models run on the child's own devices, across the platforms a family actually uses, from Android and Windows to iPhone, iPad and Mac. You can route traffic through a filtering VPN, but only to our servers or an instance you host yourself, never a third party, and it never carries raw content."),
...
const FAQ: [(&str, &str); 5] = [
    ("Is this spyware?", "No. It is openly installed, visible on the device, and can be switched off. It only ever runs on a child's own devices, with consent, and never on adults or anyone who has not agreed to it."),
    ("What data leaves the device?", "By default, none of the content. The models judge things on the device. When something needs a parent they get a short, redacted alert with no message contents in it. If you turn on the network filter, traffic goes to our servers or one you host yourself, never a third party, and still nothing raw is kept."),
    ("Can it read my child's private messages?", "It reads what is already on the screen, on the device, so it can catch grooming inside encrypted apps. It never logs keystrokes or passwords, and that text never leaves the device or gets stored."),
        ...
        section { class: "section", style: "padding-top:clamp(20px,4vh,48px);",
            div { class: "wrap",
                div { class: "prose reveal",
                    ...
                    p {
                        "So the models are small enough to run on the device, the system keeps "
