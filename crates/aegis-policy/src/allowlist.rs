...
//! and chooses **APPROVE**, aegis-policy records that the flagged item is
...
use aegis_proto::v1::{Category, ReviewDecision, ReviewScope};
...
impl AuditLog {
    ...
    fn head(&self) -> String {
        self.entries
            ...
            .unwrap_or_else(|| "aegis-audit-genesis".to_string())
    }
    /// Verify the hash chain end-to-end. Returns the index of the first tampered
    /// entry, else `Ok(())`.
    pub fn verify(&self) -> Result<(), usize> {
        let mut prev = "aegis-audit-genesis".to_string();
...
/// Self-contained SHA-256 (keeps aegis-policy dependency-free). Used only to
