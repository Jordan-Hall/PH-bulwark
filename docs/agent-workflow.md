# Aegis — Agent Spawning & Build Workflow
...
- Builders never start before their **interface contract** (Wave B + `aegis-proto`) exists.
...
| **B1 Architect** | general-purpose | worktree | Per-crate design + **`aegis-proto` gRPC/protobuf contracts** all builders code against; client/server/cluster boundaries; device-capability + offload interface. Output: `docs/design/*.md` + `aegis-proto` definitions |
...
| **C0 proto** | general-purpose | worktree | `aegis-proto` finalized (tonic/prost) — gates all client/server builders |
| **C-cluster** | general-purpose | worktree | `aegis-cluster` (membership/health/LB/work queue/shared state) — after C0 |
| **C-server** | general-purpose | worktree | `aegis-server` (role: lb\|worker\|all-in-one, hosts analyzers) — after C0, C-cluster |
| **C-client** | general-purpose | worktree | `aegis-client` (device orchestrator wiring net/agent/flow/infer) — after C0 |
| **C-net** | general-purpose | worktree | `aegis-net` (TUN + MITM + per-install CA) |
| **C-flow** | general-purpose | worktree | `aegis-flow` (classify + buffer/delay) — after C-net |
| **C-vision** | general-purpose | worktree | `aegis-vision` (small ONNX NSFW image/frame) |
| **C-audio** | general-purpose | worktree | `aegis-audio` (small ONNX explicit-audio) |
| **C-video** | general-purpose | worktree | `aegis-video` (ffmpeg pipeline) — after C-vision, C-audio |
| **C-text** | general-purpose | worktree | `aegis-text` (rules+lexicon FIRST, small classifier SECOND, no hot-path LLM) |
| **C-agent** | general-purpose | worktree | `aegis-agent` (conventional OCR + accessibility) — feeds C-text |
| **C-supervision** | general-purpose | worktree | `aegis-supervision` (family-platform connectors) |
| **C-pas** | general-purpose | worktree | `aegis-policy` + `aegis-alert` (email) + `aegis-store` (encrypted SQLite + Postgres adapter) |
| **C-infer** | general-purpose | worktree | `aegis-infer` (device detection + local-vs-cluster offload) |
| **C-ui** | general-purpose | worktree | `aegis-ui` (axum + dashboard + cluster admin) |
| **D1 Integrator** | general-purpose | worktree | Merge crates, wire `aegis-core`, single-node end-to-end smoke, then multi-node |
...
```
A1 A2 A3 ─► B1 (incl. aegis-proto contract) , B2
                 │
...
1. Target crate + its public interface from B1 + `aegis-proto` (code to the contract; don't invent APIs).
