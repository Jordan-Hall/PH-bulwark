//
//  BulwarkEngineTests.swift — bridge tests (run on a Mac with the .a linked).
//
...
import XCTest
@testable import BulwarkFilterCore

final class BulwarkEngineTests: XCTestCase {

    func testEngineBuilds() {
        XCTAssertNotNil(BulwarkEngine())
    }

    func testBenignIsAllowed() throws {
        let engine = try XCTUnwrap(BulwarkEngine())
        let (action, category) = engine.classify("are you coming to football practice tonight?",
                                                  threadID: "t1")
        XCTAssertEqual(action, .allow)
        XCTAssertEqual(category, 1) // BULWARK_APPLE_CATEGORY_SAFE
    }
    ...
    func testImageRequestIsBlocked() throws {
        ...
        XCTAssertEqual(category, 6) // BULWARK_APPLE_CATEGORY_CSAM_SUSPECTED
    }
