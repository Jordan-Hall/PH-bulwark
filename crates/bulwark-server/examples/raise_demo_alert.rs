...
//!
//! Usage:
//!   BULWARK_CLUSTER_ENDPOINT=http://127.0.0.1:8443 cargo run -p bulwark-server --example raise_demo_alert
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let endpoint = std::env::var("BULWARK_CLUSTER_ENDPOINT")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "http://127.0.0.1:8443".to_string());
    let device_id = std::env::var("BULWARK_DEMO_DEVICE_ID")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| "bulwark-proxy-local".to_string());
    let n = now_ms();
