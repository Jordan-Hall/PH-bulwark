...
//!
//! These are **not** part of the `aegis.v1` wire contract — they carry transient
//! bytes/handles that never cross a process or network boundary. They live here
//! (not in `aegis-proto`, not duplicated per crate) so `aegis-net` (which
//! *produces* a [`CapturedFlow`]) and `aegis-flow` (which turns it into
...
/// `aegis-net` for classification. (interfaces.md `CapturedFlow`.)
#[derive(Clone, Debug)]
