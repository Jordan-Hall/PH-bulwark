...
//!
//! Per the Wave C build constraints, `aegis-text` deliberately does **not**
//! depend on `aegis-core` (to avoid build-order coupling), so it cannot use the
//! shared `aegis_core::Error`/`Result` that the `Analyzer` trait nominally
...
//! reconciles once `aegis-core` lands.
