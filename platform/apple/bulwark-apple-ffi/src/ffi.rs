...
//! C declarations live in `include/bulwark_apple.h` (kept byte-for-byte in sync;
//! see `cbindgen.toml`).
//!
//! ## Fail-open invariant
//! On *any* invalid input (null engine, null text pointer, or text that is not
//! valid UTF-8) the classifier returns [`BULWARK_APPLE_ALLOW`] and logs nothing
...
/// [`bulwark_apple_engine_new`], or freeing the same pointer twice, is undefined
/// behaviour.
///
/// # Safety
/// `ptr` must be either `null` or a pointer previously returned by
/// [`bulwark_apple_engine_new`] that has not already been freed.
#[no_mangle]
pub unsafe extern "C" fn bulwark_apple_engine_free(ptr: *mut BulwarkEngine) {
    if ptr.is_null() {
        return;
    }
...
///
/// # Parameters
/// * `engine`       — an engine from [`bulwark_apple_engine_new`]; if `null`, the
///                    call fails open and returns [`BULWARK_APPLE_ALLOW`].
...
///                    code (see `BulwarkAppleCategory` in the header). Written on
...
/// `engine` must be `null` or a live pointer from [`bulwark_apple_engine_new`].
#[no_mangle]
pub unsafe extern "C" fn bulwark_apple_classify_text(
    engine: *const BulwarkEngine,
    ...
    let text = match std::str::from_utf8(text_bytes) {
        ...
        Err(_) => {
            write_category(AppleCategory::Unspecified);
            return BULWARK_APPLE_ALLOW; // fail open; log nothing sensitive
        }
    };
...
mod tests {
    ...
    fn null_engine_fails_open() {
        ...
        assert_eq!(r, BULWARK_APPLE_ALLOW);
        assert_eq!(cat, AppleCategory::Unspecified as c_int);
    }

    #[test]
    fn null_text_fails_open() {
        let e = bulwark_apple_engine_new();
        assert!(!e.is_null());
        let r = unsafe {
            bulwark_apple_classify_text(e, std::ptr::null(), 0, std::ptr::null(), 0, std::ptr::null_mut())
        };
        assert_eq!(r, BULWARK_APPLE_ALLOW);
        unsafe { bulwark_apple_engine_free(e) };
    }
    ...
    fn image_request_blocks_via_ffi() {
        ...
        assert_eq!(r, BULWARK_APPLE_BLOCK);
    ...
    fn free_null_is_noop() {
        unsafe { bulwark_apple_engine_free(std::ptr::null_mut()) };
    }
}
