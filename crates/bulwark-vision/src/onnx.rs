...
use crate::postprocess::nsfw_probability;
...
impl ExecProviderMode {
    /// Select from `BULWARK_NSFW_EP` (`auto` | `cpu` | `gpu`). Default `Auto`.
    pub fn from_env() -> Self {
        match std::env::var("BULWARK_NSFW_EP").ok().as_deref() {
            Some("cpu") => Self::Cpu,
            Some("gpu") => Self::Gpu,
            _ => Self::Auto,
        }
...
impl OnnxScorer {
    ...
    /// Construct from `BULWARK_NSFW_MODEL`, picking the model class
    /// (`BULWARK_NSFW_MODEL_CLASS`), normalization (`BULWARK_NSFW_NORM` override), and
    /// execution provider (`BULWARK_NSFW_EP`) from the environment. If the env var
    ...
    pub fn from_path_env(model_path: &str, default_input_size: u32) -> anyhow::Result<Self> {
        ...
        let input_size = if std::env::var("BULWARK_NSFW_MODEL_CLASS").is_ok() {
            class.input_size()
        } else {
            default_input_size
        };
...
/// Mirrors `bulwark-infer`'s provider mapping.
...
fn auto_select_session(model_path: &str, input_size: u32) -> anyhow::Result<Session> {
    ...
    let mut gpu = match build_session(model_path, gpu_then_cpu()) {
        Ok(s) => s,
        Err(e) => {
            tracing::info!(error = %e, "bulwark-vision: GPU session unavailable, using CPU");
            return Ok(cpu);
        }
    };
    ...
    match (gpu_ms, cpu_ms) {
        (Some(g), Some(c)) if g <= c => {
            tracing::info!(gpu_ms = g, cpu_ms = c, "bulwark-vision: GPU faster → GPU");
            Ok(gpu)
        }
        (Some(g), Some(c)) => {
            tracing::info!(gpu_ms = g, cpu_ms = c, "bulwark-vision: CPU faster → CPU");
            Ok(cpu)
        }
...
/// Normalization for a build: an explicit `BULWARK_NSFW_NORM` wins; otherwise the
/// model class default (ViT → half `[-1,1]`, MobileNet → ImageNet).
fn norm_from_env(class: ModelClass) -> Normalization {
    match std::env::var("BULWARK_NSFW_NORM").ok().as_deref() {
...
// Output postprocessing (sigmoid/softmax → nsfw probability) lives in
// `crate::postprocess`, shared with bulwark-infer's local first-pass seam.
    ...
    fn score(&self, image_bytes: &[u8]) -> f32 {
        match self.infer(image_bytes) {
            ...
            Err(e) => {
                ...
                tracing::debug!("bulwark-vision onnx: inference failed, scoring 0.0: {e}");
                0.0
            }
        }
    }
...
mod tests {
    ...
    /// `BULWARK_NSFW_MODEL` points at an existing file; otherwise it returns
