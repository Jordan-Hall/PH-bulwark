//! bulwark-cluster — SWIM membership, health, work queue, and graceful drain.
//!
...
impl Default for ClusterConfig {
    fn default() -> Self {
        Self {
            ...
            cluster_id: "bulwark-local".to_string(),
...
impl ClusterConfig {
    /// Build from `BULWARK_*` environment, falling back to [`Default`] per field. Lets
    ...
    ///
    /// - `BULWARK_NODE_ID`           — unique per node (e.g. the host IP).
    /// - `BULWARK_CLUSTER_ID`        — same across all nodes in one cluster.
    /// - `BULWARK_CLUSTER_ADDRESS`   — this node's `host:port` advertised to peers.
    /// - `BULWARK_CLUSTER_SEEDS`     — comma-separated peer `host:port`s to join.
    /// - `BULWARK_BACKPRESSURE_DEPTH`— queue depth above which Enqueue is refused.
    /// - `BULWARK_QUORUM_DSN`        — Postgres DSN for the authoritative lease store.
    pub fn from_env() -> Self {
        let mut cfg = Self::default();
        let v = |k: &str| std::env::var(k).ok().filter(|s| !s.trim().is_empty());
        if let Some(x) = v("BULWARK_NODE_ID") {
            cfg.node_id = x;
        }
        if let Some(x) = v("BULWARK_CLUSTER_ID") {
            cfg.cluster_id = x;
        }
        if let Some(x) = v("BULWARK_CLUSTER_ADDRESS") {
            cfg.address = x;
        }
        if let Some(x) = v("BULWARK_CLUSTER_SEEDS") {
            cfg.seeds = x
                .split(',')
        ...
        if let Some(x) = v("BULWARK_BACKPRESSURE_DEPTH").and_then(|s| s.parse::<u32>().ok()) {
            cfg.backpressure_depth = x;
        }
        cfg.quorum_dsn = v("BULWARK_QUORUM_DSN");
        cfg
    }
}
...
mod tests {
    use super::*;
    use bulwark_proto::v1::AnalysisRequest;
    ...
    fn cluster_config_from_env_overrides_and_defaults() {
        let keys = [
            "BULWARK_NODE_ID",
            "BULWARK_CLUSTER_ID",
            "BULWARK_CLUSTER_ADDRESS",
            "BULWARK_CLUSTER_SEEDS",
            "BULWARK_BACKPRESSURE_DEPTH",
            "BULWARK_QUORUM_DSN",
        ];
        for k in keys {
        ...
        std::env::set_var("BULWARK_NODE_ID", "lb-1");
        std::env::set_var("BULWARK_CLUSTER_ADDRESS", "10.0.0.10:8443");
        std::env::set_var("BULWARK_CLUSTER_SEEDS", "10.0.0.10:8443, 10.0.0.11:8443 ,");
        std::env::set_var("BULWARK_BACKPRESSURE_DEPTH", "1024");
        std::env::set_var("BULWARK_QUORUM_DSN", "postgres://db/bulwark");
        ...
        assert_eq!(c.quorum_dsn.as_deref(), Some("postgres://db/bulwark"));
        for k in keys {
            std::env::remove_var(k);
        }
    }
