...
//! tonic service so `aegis-server` can mount it. The exact generated trait
...
//! `aegis-server`); peers authenticate with role-scoped client certs.

use std::pin::Pin;
use std::sync::Arc;

use aegis_proto::v1::cluster_control_server::ClusterControl;
use aegis_proto::v1::{
