//! # bulwark-alert
//!
//! Guardian alerting for Bulwark: turn an [`AlertEvent`] into a clear,
...
//!
//! `docs/design/interfaces.md` defines `AlertSink` with `bulwark_core::Result`.
//! Per the Wave C build constraints this crate does not depend on `bulwark-core`,
...
mod tests {
    ...
    fn test_config() -> AlertConfig {
        AlertConfig {
            ...
            from: "Bulwark <bulwark@home.example>".to_string(),
            recipients: vec!["guardian@example.com".to_string()],
            subject_prefix: "[Bulwark]".to_string(),
    ...
    fn renders_intervention_email_with_safe_fields_only() {
        ...
        let rendered = render_event(&event, "[Bulwark]").unwrap();
    ...
    fn renders_grooming_email_with_distinct_framing() {
        ...
        let intv = render_event(&intervention_event(), "[Bulwark]").unwrap();
    ...
    fn assert_no_media_rejects_data_uri_in_context() {
        ...
        assert!(render_event(&event, "[Bulwark]").is_err());
    }
    ...
    fn digest_render_is_redacted_and_lists_all() {
        ...
        let rendered = render_digest(&events, "[Bulwark]").unwrap();
