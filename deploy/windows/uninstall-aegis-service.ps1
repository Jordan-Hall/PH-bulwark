<#
.SYNOPSIS
  Remove the Aegis Windows service (guardian, elevated).
...
Write-Host "IMPORTANT: untrust the Aegis root CA in the CHILD's user store so no MITM root lingers:"
Write-Host "  (in the child's session) certutil -delstore -user Root <Aegis-CA-fingerprint>"
