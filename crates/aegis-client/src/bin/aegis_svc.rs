//! `aegis_svc` — the SCM-managed Aegis Windows service (tamper resistance).
//!
//! Runs the Aegis filter under the Windows Service Control Manager so a Standard
//! (non-admin) child user cannot stop or delete it — the install script
//! (`deploy/windows/install-aegis-service.ps1`) locks the service DACL (`sc sdset`)
...
//! `aegis_proxy.exe` and restarts it if it exits or is killed, until the SCM stops
...
//! per-user CA. The production fix is to launch `aegis_proxy.exe` INTO the active
//! console session as the child via `WTSQueryUserToken` + `CreateProcessAsUserW`
//! (an isolated Win32 module in `aegis-net`), or to host the in-process transparent
...
fn main() {
    eprintln!(
        "aegis_svc is Windows-only (SCM service). On Linux/macOS use the systemd \
         unit / LaunchDaemon under deploy/."
    );
}

#[cfg(windows)]
const SERVICE_NAME: &str = "AegisChildSafety";
...
fn service_main(_args: Vec<std::ffi::OsString>) {
    if let Err(e) = run_service() {
        tracing::error!(error = %e, "aegis_svc failed");
    }
}
...
fn run_service() -> anyhow::Result<()> {
    ...
    // Watchdog: keep aegis_proxy.exe alive until asked to stop. Sync std only — the
...
/// (Re)launch `aegis_proxy.exe` and restart it if it exits, until `stop` is set.
#[cfg(windows)]
fn supervise(stop: &std::sync::atomic::AtomicBool) {
    use std::sync::atomic::Ordering;
    use std::time::Duration;

    let exe = match aegis_proxy_path() {
        Some(p) => p,
        None => {
            tracing::error!(
                "aegis_proxy.exe not found next to aegis_svc.exe; nothing to supervise"
            );
            return;
        }
    };
    ...
    while !stop.load(Ordering::SeqCst) {
        match std::process::Command::new(&exe).spawn() {
            Ok(mut child) => {
                tracing::info!(pid = child.id(), "aegis_proxy started under service");
                ...
                loop {
                    ...
                    match child.try_wait() {
                        Ok(Some(status)) => {
                            tracing::warn!(?status, "aegis_proxy exited; will respawn");
                            break;
                        }
                        Ok(None) => std::thread::sleep(Duration::from_millis(500)),
                        Err(e) => {
                            tracing::warn!(error = %e, "wait on aegis_proxy failed");
                            break;
                        }
                    }
                }
            }
            Err(e) => tracing::warn!(error = %e, "failed to spawn aegis_proxy; retrying"),
        }
...
/// Resolve `aegis_proxy.exe` next to this service exe (both install into the same
/// `C:\Program Files\Aegis\` directory).
#[cfg(windows)]
fn aegis_proxy_path() -> Option<std::path::PathBuf> {
    let dir = std::env::current_exe().ok()?.parent()?.to_path_buf();
    let candidate = dir.join("aegis_proxy.exe");
