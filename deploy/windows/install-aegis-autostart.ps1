<#
.SYNOPSIS
  Install Aegis child-safety auto-start on Windows (tamper resistance, item 5).

.DESCRIPTION
  Registers a Scheduled Task that launches the Aegis filter (aegis_proxy.exe) at
  ...
  THE MODEL (read docs/design/tamper-protection.md §5):
    ...
    3. Uninstalling Aegis then requires admin elevation the child doesn't have.
...
.PARAMETER ExePath
  Full path to aegis_proxy.exe. Defaults to the copy next to this script.

.EXAMPLE
  # From an elevated PowerShell:
  .\install-aegis-autostart.ps1 -ExePath 'C:\Program Files\Aegis\aegis_proxy.exe'
#>
[CmdletBinding()]
param(
    [string]$ExePath = (Join-Path $PSScriptRoot 'aegis_proxy.exe'),
    [string]$TaskName = 'AegisChildSafety'
)
...
if (-not (Test-Path $ExePath)) {
    throw "aegis_proxy.exe not found at '$ExePath'. Pass -ExePath."
}
