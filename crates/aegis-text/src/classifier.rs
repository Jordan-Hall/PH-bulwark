...
mod tests {
    ...
    fn distilbert_classifier_golden_when_model_present() {
        ...
        //   AEGIS_GROOMING_DISTILBERT=<...>/grooming_detector_v2.onnx  (sidecar alongside)
        //   AEGIS_GROOMING_TOKENIZER=<...>/tokenizer/tokenizer.json
        // (Python-side Layer-1 faithfulness is verified separately: windowed AUC 0.9837.)
        let (Ok(model), Ok(tok)) = (
            std::env::var("AEGIS_GROOMING_DISTILBERT"),
            std::env::var("AEGIS_GROOMING_TOKENIZER"),
        ) else {
