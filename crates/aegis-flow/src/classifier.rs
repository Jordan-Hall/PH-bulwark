...
//!
//! interfaces.md declares `FlowClassifier` in terms of `aegis_core::Result`;
//! this crate depends on `aegis-core`, so we use that `Result` directly (the
...
use aegis_proto::v1::{Action, InlineMedia, MediaKind, SourceChannel, TextSpan};
...
impl DefaultFlowClassifier {
    ...
    /// `due_segments`, observe back-pressure). The orchestrator (`aegis-client`)
