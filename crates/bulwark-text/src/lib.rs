//! # bulwark-text — deterministic grooming rules + lexicon engine (PRIMARY), plus
//! adult-text detection, behind the `Analyzer` contract.
//!
//! ## Minimal-AI design
//! Bulwark is **rules-first** (PLAN.md §0b). This crate's real detector is the
...
//! a rule signal (sets [`bulwark_proto::GroomingSignal::classifier_backed`]); it
...
//! [`TextSpan`](bulwark_proto::TextSpan) → [`Verdict`](bulwark_proto::Verdict) with a
//! populated [`GroomingSignal`](bulwark_proto::GroomingSignal). Conversation state
...
mod integration_tests {
    ...
    use bulwark_proto::{Action, Category, GroomingRule, Severity};
    ...
    async fn analyze_stream_yields_one_verdict_per_request() {
        use bulwark_proto::{AnalysisRequest, MediaKind};
