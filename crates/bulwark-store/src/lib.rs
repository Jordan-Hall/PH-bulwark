//! # bulwark-store — redacted event/verdict persistence (the [`Store`] contract).
//!
...
pub fn open_in_memory() -> bulwark_core::Result<std::sync::Arc<dyn Store>> {
    #[cfg(feature = "sqlite")]
    {
    ...
    {
        return Err(bulwark_core::Error::Other(anyhow::anyhow!(
            "bulwark-store: no in-memory store backend enabled (enable `portable` or `sqlite`)"
        )));
    }
}
...
pub trait Store: Send + Sync {
    ...
    /// The blob is content-free by the `bulwark-text` producer contract.
    async fn thread_state(&self, thread_id: &str) -> bulwark_core::Result<Option<Vec<u8>>>;

    /// Upsert a conversation-state blob.
    async fn put_thread_state(&self, thread_id: &str, state: &[u8]) -> bulwark_core::Result<()>;

    /// Verify the tamper-evident audit hash chain. `Ok(())` = intact; an error
    /// (`bulwark_core::Error` wrapping [`StoreError::Integrity`]) names the first
    /// tampered row. Default impl: assume integrity is backend-checked elsewhere.
    async fn verify_audit_chain(&self) -> bulwark_core::Result<()> {
        Ok(())
    }

    /// Run the retention auto-purge at `now_ms`. Default impl: no-op (a backend
    /// without retention overrides this).
    async fn purge_expired(&self, _now_ms: i64) -> bulwark_core::Result<PurgeReport> {
...
impl Store for SqliteStore {
    ...
    async fn purge_expired(&self, now_ms: i64) -> bulwark_core::Result<PurgeReport> {
...
mod store_trait_tests {
    use super::*;
    use bulwark_proto::v1::{Action, Category, Severity, Verdict};
