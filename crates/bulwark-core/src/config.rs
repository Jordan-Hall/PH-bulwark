...
//!
//! Every Bulwark binary (`bulwark-client`, `bulwark-server`, …) loads one [`Config`].
//! The layering, lowest-to-highest precedence, is:
//!
//! 1. [`Config::default`] — compiled-in safe defaults (fail-safe per PLAN §3).
//! 2. A TOML file (path passed to [`Config::load`], or `BULWARK_CONFIG`).
//! 3. Environment variables prefixed `BULWARK_`, nested with `__`
//!    (e.g. `BULWARK_SMTP__HOST=mail.example.com`, `BULWARK_CLUSTER__ROLE=worker`).
//!
...
pub const CONFIG_PATH_ENV: &str = "BULWARK_CONFIG";

/// Prefix for environment-variable overrides (`BULWARK_SMTP__HOST`, …).
pub const ENV_PREFIX: &str = "BULWARK_";

/// Top-level Bulwark configuration. One per process.
///
...
/// Guardian-alert email settings consumed by `bulwark-alert`.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(default)]
...
/// Filesystem paths to policy + lexicon assets (consumed by `bulwark-policy` /
/// `bulwark-text`). Paths only — this crate does not parse them.
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
#[serde(default)]
...
/// the loading: analyzer crates (`bulwark-vision`/`-audio`/`-text`) load and verify
...
impl Config {
    ...
    pub fn figment(path: Option<&Path>) -> Figment {
        ...
        // Env overrides last (highest precedence). `BULWARK_` prefix, `__` nests:
        // `BULWARK_SMTP__HOST` → smtp.host, `BULWARK_CLUSTER__SEED_NODES` (CSV-ish).
        fig.merge(Env::prefixed(ENV_PREFIX).split("__"))
    }

    /// Load configuration, layering a TOML file at `path` (if given) over the
    /// compiled-in defaults, then applying `BULWARK_`-prefixed env overrides.
    ///
    ...
    /// Load using the path from `$BULWARK_CONFIG` if set, else defaults + env only.
    ...
    fn loads_toml_and_env_layers() {
        Jail::expect_with(|jail| {
            jail.create_file(
                ...
                [models]
                model_dir = "/opt/bulwark/models"
                "#,
            )?;

            // Env override should win over the TOML value.
            jail.set_env("BULWARK_CLUSTER__BIND_ADDR", "0.0.0.0:9999");
            jail.set_env("BULWARK_SMTP__PASSWORD", "from-env");

            let cfg = Config::load(Some(Path::new("bulwark.toml"))).expect("config should load");
            ...
            assert_eq!(
                ...
                Some(Path::new("/opt/bulwark/models"))
            );
