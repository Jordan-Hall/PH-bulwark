//! # bulwark-core
//!
//! The shared foundation every other Bulwark crate imports: configuration,
//! the workspace-wide error type, local diagnostic logging, device-capability
//! detection, and the identifier newtypes that wrap the `bulwark.v1` wire ids.
//!
...
//! ## Quick start
//! ```no_run
//! use bulwark_core::{Config, init_tracing, detect_device_profile};
//!
//! # fn main() -> bulwark_core::Result<()> {
//! let config = Config::load_default()?;
//! config.validate()?;
//! init_tracing(&config.logging)?;
//!
//! let profile = detect_device_profile(); // hand to bulwark-infer's OffloadRouter
...
/// Re-export of the wire contract so downstream crates can `use bulwark_core::proto`
/// without a separate `bulwark-proto` import when they only need a few types.
